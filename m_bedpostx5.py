...
workflow.connect([
    ...
    # input to bedp
    (flip_bvectors_node, bedp, [('bvecs_out', 'bvecs')]),

    (template_source,bedp,[('T1_mask', 'mask')]),
    
    (data_source,bedp,[('dwi', 'dwi'), ('bval', 'bvals')]),

    # bedp -- > pbx2
    (bedp, pbx2,
         [
          ('mean_thsamples', 'thsamples'),
          ('mean_fsamples', 'fsamples'),
          ('mean_phsamples', 'phsamples'),
         ]
     ),

     (bedp, data_sink,
         [
          ('dyads_dispersion', 'dyads_dispersion'),
          ('mean_S0samples', 'mean_S0samples'),
          ('mean_dsamples', 'mean_dsamples'),
