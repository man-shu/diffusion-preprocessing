...
def create_workflow(subject_id):
    ...
    infosource.iterables = [('subject_id', subject_id), ('visit', visits), ('session', sessions)]
    ...
    return tractography_wf



   

def run_workflow(value_A):
    workflow = create_workflow(value_A)

    # Run the workflow
    workflow.run(plugin='MultiProc', plugin_args={'n_procs':20, 'memory_gb': 8,'dont_resubmit_completed_jobs':True})


'''tractography_wf.run(plugin='SLURM',
                     plugin_args={
...
'''tractography_wf.run(plugin='SLURMGraph',
                     plugin_args={
                         'dont_resubmit_completed_jobs':
				 True,
                         'sbatch_args':
...
if __name__ == "__main__":
    ...
    values = config['DEFAULT']['id_list'].split(" ")

    parser = argparse.ArgumentParser()
    parser.add_argument("--id_list", type=int, help="Path to id_list")

    args, _ = parser.parse_known_args()

    #run_workflow(args.id_list)


    # Call the run_workflow function with each value from the config
    for subject_id in values:
        run_workflow(subject_id)
