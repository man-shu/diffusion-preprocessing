...
def shrink_surface(surface_file, reference, distance, outfile):
    ...
    signed_distance = signed_distance_si.get_data()

    # Transform the points to voxels
    mm_to_voxel_affine = numpy.linalg.inv(wm_mask.affine)
    vertices_voxel = nibabel.affines.apply_affine(mm_to_voxel_affine,
                                                  surface.vertices)

    # Push vertices in the white matter following the gradient
    gradient = numpy.array(numpy.gradient(-signed_distance))

    shrinked_vertices_voxel = [spatial.grad_descend(p, gradient,
                                                    distance, voxel_size)
                               for p in vertices_voxel]

    # Bring back to mm
    shrinked_vertices = nibabel.affines.apply_affine(wm_mask.affine,
...
def command_line_main():
    ...
    parser.add_argument('-reference', dest='reference', type=str,
                        required=True, help='reference volume')

    parser.add_argument('-mm', dest='distance', type=float, required=True,
